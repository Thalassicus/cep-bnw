...
-- Cope: [ WORKEDTILES ] Listen for shift keydown as well as mouseover movement further down
    elseif ( wParam == Keys.VK_SHIFT ) and (Civup.HIGHLIGHT_WORKED_CITY_TILES == 0) then
...
function OnMouseOverCityPlot( hexX, hexY )
	if (Civup.HIGHLIGHT_WORKED_CITY_TILES == 1) or (Civup.HIGHLIGHT_WORKED_CITY_TILES == 0 and UIManager:GetShift()) then
		local plot = Map.GetPlot( hexX, hexY );
		
		if plot and plot:IsCity() and plot:GetOwner() == Game.GetActivePlayer() then
			...
			if plot:GetOwner() == city:GetOwner() then
				...
				for i = 0, city:GetNumCityPlots() - 1, 1 do
					local adjPlot = city:GetCityIndexPlot( i );
					if city:IsWorkingPlot(adjPlot)  then
						Events.SerialEventHexHighlight( ToHexFromGrid( Vector2( adjPlot:GetX(), adjPlot:GetY() ) ), true, hColor, "GroupBorder");
					end
				end
			end
