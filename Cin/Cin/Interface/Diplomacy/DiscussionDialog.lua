    ...
    if (bMyMode) then
		...
		if (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_HUMAN_INVOKED) then
			...
			if (g_iInvokedDiscussionMode == g_iModeDiscussionRoot) then
				...
				if (Controls.Button7 and activePlayer:GetNegativeReligiousConversionPoints(g_iAIPlayer) > 0 and not pAIPlayer:IsAskedToStopConverting(iActivePlayer)) then
				...
				if (not pAIPlayer:IsDontSettleMessageTooSoon(iActivePlayer)) then
					...
					if not Controls.Button7 then
						strButton3Text = strButton4Text;
						strButton4Text = -1;
					end
				end
				...
				if (pAIPlayer:GetTeam() ~= Players[iActivePlayer]:GetTeam()) then
					...
					if (not pAIPlayer:IsDoFMessageTooSoon(iActivePlayer)) then
						strButton5Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_MESSAGE_DEC_FRIENDSHIP" );
						strButton5Tooltip = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_MESSAGE_DEC_FRIENDSHIP_TT" );
						if not Controls.Button7 then
							strButton4Text = strButton5Text;
							strButton4Tooltip = strButton5Tooltip;
							strButton5Text = -1;
							strButton5Tooltip = "";
						end
					...
					if(not activePlayer:IsDenouncedPlayer(iPlayer)) then
						strButton6Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_DENOUNCE", strLeaderName );
						strButton6Tooltip = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_DENOUNCE_TT" );
						if not Controls.Button7 then
							strButton5Text = strButton6Text;
							strButton5Tooltip = strButton6Tooltip;
							strButton6Text = -1;
							strButton6Tooltip = "";
						end
		 			end
				...
				strButton7Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_MESSAGE_DECLARE_WAR" );
		 		if Controls.Button7 then
		 			Controls.Button7:SetDisabled(true);
		 		else
					strButton6Text = strButton7Text;
					strButton7Text = -1;
...
function OnButton3()
	...
	if (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_HUMAN_INVOKED) then
		if (g_iInvokedDiscussionMode == g_iModeDiscussionRoot) then
			if Controls.Button7 then
				-- Ask the AI player not to spread religion
	...
	if (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_HUMAN_INVOKED) then
		if (g_iInvokedDiscussionMode == g_iModeDiscussionRoot) then
			if Controls.Button7 then
				...
				discussWorkingTogether(pAIPlayer);
			end
			-- Tell the player we're done working with him
			--elseif (pAIPlayer:IsDoF(iActivePlayer)) then
...
function discussSettlingNearby()
...
function discussWorkingTogether(pAIPlayer)
	-- Ask the AI player to work together
	if (not pAIPlayer:IsDoFMessageTooSoon(iActivePlayer)) then
